...
class ContextBuilder:
    ...
    def _build_game_state(self, current_actor_id: Optional[str] = None) -> str:
        ...
        if self.session.is_multiplayer:
            state["multiplayer_context"] = self._build_multiplayer_context(current_actor_id)
    ...
    def _build_multiplayer_context(self, current_actor_id: Optional[str] = None) -> dict:
        mp = (self.session.game_state or {}).get("multiplayer", {}) or {}
        ...
        pending_by_group = mp.get("pending_actions_by_group") if isinstance(mp.get("pending_actions_by_group"), dict) else {}
        ...
        if actor_user_id:
            actor_group = next(
                (group for group in normalized_groups if actor_user_id in (group.get("member_user_ids") or [])),
                None,
            )
