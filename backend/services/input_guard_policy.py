...
ENGLISH_RULE_VIOLATION_PATTERNS = (
    r"\b(auto|automatic|automatically|guaranteed|always|instantly|directly)\b.*\b(hit|crit|critical|succeed|success|pass|kill|destroy)\b",
    r"\b(skip|ignore|bypass|without|no need for)\b.*\b(check|save|saving throw|roll|rules?|dc|ac)\b",
    r"\b(give|grant|add|restore|heal)\b.{0,40}\b(me|myself|my character|us|the party)\b.{0,40}\b(full|infinite|unlimited|999|9999|massive)\b.{0,40}\b(hp|health|gold|money|xp|experience)\b",
    r"\b(set|change|reduce|lower|clear)\b.{0,40}\b(enemy|enemies|monster|monsters|boss)\b.{0,40}\b(ac|hp|health|stats?)\b",
)
...
RULE_VIOLATION_RE = re.compile(
    "|".join(RULE_VIOLATION.patterns + ENGLISH_RULE_VIOLATION_PATTERNS),
    re.IGNORECASE,
)
