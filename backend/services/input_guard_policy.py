...
COMMON_GAME_ACTION = PatternGroup("legal_rule_terms", "常见游戏行动", (
    r"(我|我们|角色|队伍).{0,12}(检查|调查|观察|搜索|聆听|靠近|走向|进入|打开|推开|拿起|拾起|交谈|询问|说服|威吓|潜行|攻击|施法|躲避|冲刺|撤离|帮助|使用)",
    r"^(检查|调查|观察|搜索|聆听|靠近|走向|进入|打开|推开|拿起|拾起|交谈|询问|说服|威吓|潜行|攻击|施法|躲避|冲刺|撤离|帮助|使用)",
    r"(祭坛|符文|门|箱子|尸体|脚印|陷阱|守卫|村长|商人|敌人|哥布林|骷髅|宝箱|房间|通道|洞穴|墙壁|地面)",
))
...
COMMON_GAME_ACTION_RE = COMMON_GAME_ACTION.compile()
...
def fast_pass_in_game(action: str) -> GuardResult | None:
    """Conservatively skip LLM guard for short, ordinary in-game actions."""
    normalized = (action or "").strip()
    if not normalized or len(normalized) > 120:
        return None
    if INJECTION_RE.search(normalized) or OFF_TOPIC_RE.search(normalized) or RULE_VIOLATION_RE.search(normalized):
        return None
    if COMMON_GAME_ACTION_RE.search(normalized):
        return {"verdict": "in_game", "reason": "常见游戏行动快速放行", "refusal": ""}
    return None
...
def classify_by_local_rules(action: str) -> GuardResult | None:
    ...
    fast_pass = fast_pass_in_game(action)
    if fast_pass:
        return fast_pass
    return None
