...
async def execute_natural_language_combat_action(
    *,
    ...
    trace = AILatencyTrace(
        ...
        user_id=getattr(player, "user_id", None) or getattr(session, "user_id", "") or "",
        metadata={
            "mode": "combat",
            "round": getattr(combat_state, "round_number", 0),
        },
    )
    parsed: dict[str, Any] = {"actions": []}
    execution = None
    ...
    try:
        ...
        trace.log_success(extra={
            "parsed_actions": len(parsed.get("actions") or []),
            "action_results": len(execution.action_results),
            "combat_ended": bool(combat_over),
        })
        return {
            "type": "combat_action",
            "narrative": narrative,
        ...
        trace.log_error(error=exc, extra={
            "parsed_actions": len(parsed.get("actions") or []),
            "action_results": len(execution.action_results) if execution else 0,
        })
        raise
