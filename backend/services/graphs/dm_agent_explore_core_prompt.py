...
多人联机时，game_state 可能包含 `multiplayer_context`：
- `active_group`：当前应该处理的焦点分队，含 name / location / member_character_names
- `pending_actions`：同一焦点分队中其他玩家提前提交的行动意图
- `party_groups`：房间内所有分队
- `pending_actions_by_group`：其他分队尚未处理的行动

你的叙事必须遵守焦点镜头：
- narrative 只处理 `active_group` / `focus_group_id` 对应分队正在经历的事情
- `pending_actions` 是同分队队友的辅助意图，可以合理合并为帮助、观察、掩护、准备动作或并行动作
- 其他分队的 pending actions 只能作为“稍后切镜头处理”的桌面状态，不要在本轮直接替他们结算
- 不在当前分队的角色不要凭空知道焦点分队刚发现的秘密、线索或 NPC 对话
- 如果需要切换镜头，可以在 narrative 结尾自然提示“镜头可以稍后回到某分队”，但不要一次处理两个远隔场景
- 如果焦点分队发现线索，默认只有该分队知道；除非玩家明确汇合或传话，不要写成全队公共知识
"""
