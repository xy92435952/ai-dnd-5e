...
async def test_multiplayer_guest_cutting_words_damage_reduces_damage_and_broadcasts_state(
    client,
    db_session,
    engine,
    sample_module,
    ...
    async def fake_call_dm_agent(**kwargs):
        return {
            "result": json.dumps({
                "action_type": "combat_start",
                "narrative": "A cutting wind ripples through the ward.",
                "player_choices": [],
                "companion_reactions": "",
                "state_delta": {
                    "combat_trigger": True,
    ...
    async def fake_get_ai_decision(**kwargs):
        return {"action_type": "attack", "target_id": guest_char.id, "reason": "test cutting words damage reaction"}
    ...
    guest_char.char_class = "Bard"
    guest_char.subclass = "Lore"
    guest_char.level = 5
    guest_char.hp_current = 20
    guest_char.class_resources = {"bardic_inspiration_remaining": 2}
    guest_char.derived = {
    ...
    try:
        ...
        ai_body = ai_result.json()
        assert ai_body["target_id"] == guest_char.id
        assert ai_body["target_new_hp"] == 11
        assert ai_body["player_can_react"] is False
        assert ai_body["reaction_prompt"] is None

        ai_update = await _wait_for_event(guest_ws, "combat_update", timeout=2, start_index=before_ai_count)
        assert ai_update["combat"]["entities"][guest_char.id]["hp_current"] == 11
        assert ai_update["player_can_react"] is True
        assert ai_update["reaction_prompt"]["reactor_character_id"] == guest_char.id
        cutting_words_damage = next(
            reaction
            for reaction in ai_update["reaction_prompt"]["available_reactions"]
            if reaction["type"] == "cutting_words_damage"
        )
        assert cutting_words_damage["damage_roll_before"] == 9
        ...
        reaction = await client.post(
            ...
            json={
                ...
                "target_id": enemy["id"],
                "character_id": guest_char.id,
                "cutting_words_roll": 4,
            },
        )
        assert reaction.status_code == 200, reaction.text
        reaction_body = reaction.json()
        assert reaction_body["reaction_effect"]["damage_roll_before"] == 9
        assert reaction_body["reaction_effect"]["damage_roll_after"] == 5
        assert reaction_body["reaction_effect"]["damage_prevented"] == 4
        assert reaction_body["dice_result"]["type"] == "reaction"
        assert reaction_body["dice_result"]["reaction_type"] == "cutting_words_damage"
        assert reaction_body["dice_result"]["damage_roll_before"] == 9
        assert reaction_body["dice_result"]["damage_roll_after"] == 5
        assert reaction_body["dice_result"]["damage_prevented"] == 4
        assert reaction_body["special_action"] == reaction_body["dice_result"]

        await db_session.refresh(guest_char)
        assert guest_char.class_resources["bardic_inspiration_remaining"] == 1

        reaction_update = await _wait_for_event(host_ws, "combat_update", timeout=2, start_index=before_reaction_count)
        assert reaction_update["actor_id"] == guest_char.id
        assert reaction_update["action"] == "reaction"
        assert reaction_update["reaction_type"] == "cutting_words_damage"
        assert reaction_update["reaction_effect"] == reaction_body["reaction_effect"]
        assert reaction_update["target_state"] == reaction_body["target_state"]
        assert reaction_update["actor_state"] == reaction_body["target_state"]
        assert reaction_update["remaining_slots"] == reaction_body["remaining_slots"]
        ...
        assert guest_refresh.status_code == 200, guest_refresh.text
        assert "pending_attack_reaction" not in host_refresh.json()["turn_states"][guest_char.id]
        assert "pending_attack_reaction" not in guest_refresh.json()["turn_states"][guest_char.id]
    finally:
        await host_ws.disconnect()
        await guest_ws.disconnect()
        await asyncio.gather(host_task, guest_task, return_exceptions=True)
