...
async def test_seeded_feather_fall_variant_decline_applies_saved_damage(
    client,
    db_session,
):
    """Declining the deployment-smoke prompt should apply the saved fall damage without spending the slot."""
    result = await seed_smoke_scenario(
        db_session,
        slug="feather fall decline http qa",
        variant="feather-fall",
    )
    ...
    body = declined.json()
    assert body["type"] == "exploration_reaction"
    assert body["action"] == "reaction_declined"
    assert body["reaction_effect"]["type"] == "feather_fall"
    assert body["target_state"]["hp_current"] == data["player"]["hp_max"] - 6
    assert body["caster_state"]["spell_slots"]["1st"] == 1
    ...
    assert "pending_exploration_reaction" not in session.game_state
    assert hero.hp_current == data["player"]["hp_max"] - 6
    assert companion.spell_slots["1st"] == 1

    after = await client.get(f"/game/sessions/{result.session_id}", headers=headers)
    assert after.status_code == 200, after.text
    after_data = after.json()
    assert "pending_exploration_reaction" not in after_data["game_state"]
    assert after_data["player"]["hp_current"] == data["player"]["hp_max"] - 6
    ...
    assert any("lets the Feather Fall window pass" in log.content for log in logs)
    assert any("deals 6 damage" in log.content for log in logs)
