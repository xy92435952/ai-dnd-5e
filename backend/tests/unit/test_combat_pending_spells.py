...
def test_complete_pending_spell_marks_action_cantrip_action():
    from api.combat.pending_spells import complete_pending_spell
    from models import CombatState
    ...
    updated = complete_pending_spell(combat, "char-1", is_cantrip=True)

    assert "pending_spell" not in updated
    assert updated["action_used"] is True


def test_complete_pending_spell_marks_bonus_action_cost():
    from api.combat.pending_spells import complete_pending_spell
    from models import CombatState
    ...
    updated = complete_pending_spell(
        combat,
        "char-1",
        is_cantrip=False,
        action_cost="bonus",
