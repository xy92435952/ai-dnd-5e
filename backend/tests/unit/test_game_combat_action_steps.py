...
def test_execute_parsed_mobile_dash_marks_difficult_terrain_ignore():
    from services.game_combat_action_executor import execute_parsed_combat_actions

    actor = SimpleNamespace(conditions=[], hp_current=10)
    combat_state = SimpleNamespace(entity_positions={"hero-1": {"x": 0, "y": 0}}, turn_states={})
    turn_state = {
        "action_used": False,
        "movement_used": 0,
        "movement_max": 8,
    ...
    result = execute_parsed_combat_actions(
        ...
        player=actor,
        player_id="hero-1",
        player_derived={"feat_effects": {"Mobile": {"mobile": True}}},
        turn_state=turn_state,
        move_remaining=8,
        combat_service=FakeCombatService(),
        move_toward=lambda *_args: None,
        save_turn_state=save_turn_state,
        check_attack_range=lambda *_args: (True, 1, None),
        distance=lambda *_args: 1,
    )

    assert result.executed_action_types == ["dash"]
    assert turn_state["movement_max"] == 16
    assert turn_state["action_used"] is True
    assert turn_state["mobile_ignores_difficult_terrain"] is True
    assert saved_states[-1]["mobile_ignores_difficult_terrain"] is True
