...
HEAD=$(alembic heads 2>&1 | tail -1 || echo "")
if [ -n "$CUR" ] && [ "$CUR" = "$HEAD" ]; then
    echo "  ✓ 已是最新版本，无需迁移"
elif echo "$CUR" | grep -q "20260417_0001_baseline_v08"; then
    echo "  从 baseline 升级..."
    alembic upgrade head
elif echo "$CUR" | grep -q "20260417_0002_multiplayer\|20260509_0003_game_log_visibility\|20260512_0004_game_log_table_decision"; then
    echo "  从当前版本升级到 head..."
    alembic upgrade head
else
...
append_if_missing "BETA_MAX_USERS" "50"
append_if_missing "BETA_MAX_WS_CONNECTIONS" "80"
append_if_missing "RATE_LIMIT_ENABLED" "true"
append_if_missing "RATE_LIMIT_DEFAULT_PER_MINUTE" "120"
append_if_missing "RATE_LIMIT_AUTH_PER_MINUTE" "20"
append_if_missing "RATE_LIMIT_GAME_PER_MINUTE" "30"
append_if_missing "MODULE_PARSE_MAX_CONCURRENT" "1"
append_if_missing "MODULE_PARSE_MAX_BACKLOG" "5"

if grep -q "^DATABASE_URL=sqlite" "$ENV_FILE"; then
    echo "  ⚠ DATABASE_URL 仍是 SQLite。当前生产服务器应使用 PostgreSQL，请改为 postgresql+asyncpg://..."
fi
if ! grep -q "^LANGGRAPH_DB_URL=postgresql" "$ENV_FILE"; then
    echo "  ⚠ LANGGRAPH_DB_URL 未指向 PostgreSQL。生产环境建议与主库同 PostgreSQL。"
fi
