...
Combat `CombatHudCombatLog`, Combat `TurnBanner`, Combat `InitiativeRibbon` HP tick chrome, Combat `TargetCard` HP meter chrome, Combat `CombatStage`,
Combat `CombatTacticalContextPanel`, Combat `BattlefieldLegend`, Combat `CombatOutcomeOverlay`,
Combat `CombatDeathSavePanel` panel/dot tones, Combat `CombatQuickInventory`, Combat `IsoBattlefield` unit tone/HP fill chrome, shared inventory gold strip/action controls/tags/row/shop/meta tones,
...
Room page shell and member/AI/action sections,
