...
Combat `SpellModal` cast-plan dialog description linkage,
...
Combat `CombatHudCombatLog` attack modifier-source rows, Combat `TurnBanner`, Combat `InitiativeRibbon` HP tick chrome, Combat `TargetCard` HP meter chrome, Combat `CombatStage` wash/float-text chrome,
...
values and fail fast when a following flag or empty inline value is provided.
Unknown `--` options also fail fast, while `--help` prints usage without
starting the temporary backend/frontend/browser. Browser-smoke evidence cannot
silently fall back to the accept path or default date tag after a mistyped
command.
...
- 2026-06-23: `node scripts\feather_fall_adventure_browser_smoke.mjs --artifact-tag 20260623-evidence` passed after the manifest began recording the prompt's `dialogName` / `dialogDescription`. The run restored the seeded Adventure prompt, verified the accessible dialog name was `Feather Fall` and the described text included `Prevents 6 fall damage`, clicked `Cast Feather Fall`, confirmed `pending_exploration_reaction` cleared, kept player HP at `28/28`, spent the caster's `1st` slot to `0`, wrote `artifacts\browser-feather-fall-adventure-prompt-20260623-evidence.png` / `artifacts\browser-feather-fall-adventure-resolved-20260623-evidence.png`, and `node scripts\verify_stage7_evidence.mjs artifacts\browser-feather-fall-adventure-manifest-20260623-evidence.json` verified the manifest.
...
bad repository or missing run ID, still fail immediately so CI triage does not
...
screenshot paths. For multiplayer load result
...
verifies it. If `RUN_STAGE7_POSTDEPLOY_HEALTHCHECK=1` and
`RUN_STAGE7_EVIDENCE_GATE=1` are both set without
...
$env:RUN_STAGE7_POSTDEPLOY_HEALTHCHECK='1'
$env:RUN_STAGE7_EVIDENCE_GATE='1'
$env:STAGE7_POSTDEPLOY_HEALTH_URLS='http://127.0.0.1:8000/health https://your-domain.example/api/health'
$env:STAGE7_POSTDEPLOY_LOG_FILES='artifacts\server-journal-YYYYMMDD.log artifacts\nginx-error-YYYYMMDD.log'
& 'C:\Program Files\Git\bin\bash.exe' scripts/check.sh
```
