...
`npm run test:stage7:reaction` includes Home hub shell, Home module/save cards, ClassGallery card chrome, RoomLobby shell, RoomLobby DM-style selection, CharacterCreate shell, shared `DiceRollerOverlay`, shared `LegendForge`, shared `Ornaments`, CharacterSheet shell, CharacterSheet HP tone,
