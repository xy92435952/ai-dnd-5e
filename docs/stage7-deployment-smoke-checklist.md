...
`npm run test:stage7:reaction` includes Home hub shell, RoomLobby shell, CharacterCreate shell, CharacterSheet shell,
...
coordination, multiplayer speak-turn, room page shell, room multiplayer readiness status, room member management cards, room AI companion cards, room lifecycle action controls, character-create feat-choice step, character-create companion details, character-create spell choices, combat smite prompt, combat maneuver modal, combat legendary/lair action prompt, combat overlay alerts, combat multiplayer turn status, shared multiplayer session status, shared websocket status pill, shared Magic Initiate choice fields, combat spell modal shell, combat spell modal tabs, combat spell modal list, combat spell modal actions, combat quick-inventory, and multiplayer table-decision notice coverage. Keep new
