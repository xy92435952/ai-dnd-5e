...
- 2026-06-23: `node scripts\feather_fall_adventure_browser_smoke.mjs --artifact-tag 20260623-selector` passed after the dialog selector switched to the `aria-labelledby` Feather Fall title. The run restored the seeded Adventure prompt, verified the described text still included `Prevents 6 fall damage`, clicked `Cast Feather Fall`, confirmed `pending_exploration_reaction` cleared, kept player HP at `28/28`, spent the caster's `1st` slot to `0`, wrote `artifacts\browser-feather-fall-adventure-prompt-20260623-selector.png` / `artifacts\browser-feather-fall-adventure-resolved-20260623-selector.png`, and `node scripts\verify_stage7_evidence.mjs artifacts\browser-feather-fall-adventure-manifest-20260623-selector.json` verified the manifest.
