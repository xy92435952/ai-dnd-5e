...
**Status Sync 2026-05-21:** The first playable version is implemented in the current tree. Backend state helpers now live behind `room_service` facades with focused group modules, `RoomStateUpdated` carries full room snapshots, Adventure / Combat realtime handlers consume `room_state_updated`, and `MultiplayerPartyPanel` is wired into Adventure. Readiness/focus controls and table-decision flow were added after the original plan, so the implementation is slightly broader than this checklist.
