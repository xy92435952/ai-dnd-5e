...
| [~] | P0 | Incapacitation gates | Action/reaction validators exist; reaction already-resolved paths now still reject 0 HP, dead, stable, or stunned actors. 2026-05-30: exploration `/game/action` and multiplayer AI takeover now route through the same `assert_can_act` life-state gate before DM execution, so dying/stable/dead/paralyzed actors cannot bypass combat/skill-check restrictions through free-text or takeover actions; covered in `backend/tests/integration/test_combat_rules_endpoints.py`. | Every action endpoint uses one common rule state to reject incapacitated, dead, stunned, paralyzed, etc. |
...
- [x] Show hover/selection preview for hit chance, damage range, cover, advantage/disadvantage, and resource cost. 2026-05-29: target card and attack skill tooltip now share preview rows for hit chance, crit chance, damage dice/expected damage, min/max damage range, cover/effective AC, advantage/disadvantage, attack bonus, and action/resource cost. Richer spell-specific preview remains a future depth item.
