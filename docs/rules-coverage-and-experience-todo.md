...
- [~] Add target validation before submit, not only backend error after click. 2026-05-30: skill bar now honors backend skill metadata (`requires_target`, `target_type`, and nested targeting hints) before submit for common target-required actions; spell cast modal pre-blocks missing targets/centers, AoE casts submit explicit target ids gathered from preview cells, and battlefield clicks can lock an AoE center so leaving the grid no longer clears the chosen point. Full multi-target/ground-point polish still needs deeper validation.
