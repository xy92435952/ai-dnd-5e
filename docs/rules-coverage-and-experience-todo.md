...
| [~] | P1 | Traps and investigation | Backend passive discovery exists for hidden traps, secret doors, clues, and mechanisms. Triggered trap resolution rolls configured saves and full/half damage, and `state_delta.trap_triggers` can now apply trap damage/failed conditions to persisted characters. Disarm, attack-roll traps, and persistent trap state remain. | Detect, disarm, trigger, save/attack, damage, and persistent trap state are represented. |
