...
- [~] Add cleanup for stale pending reactions, stale group readiness, and abandoned WS connections. Combat reaction decline now clears pending attack/spell prompts, and already-resolved reactions are safe to resubmit; 2026-05-30: group action submission now resets every member in that group back to `drafting`, so old ready/waiting confirmations cannot make a newly changed plan auto-resolve, covered by `backend/tests/unit/test_room_multiplayer_state.py`. 2026-05-30: stale WS cleanup now detects members whose `last_seen_at` is older than the disconnect window, closes their sockets, marks them offline, and broadcasts `member_offline`, covered by `backend/services/ws_cleanup_service.py`, `backend/tests/unit/test_ws_manager.py`, and `backend/tests/integration/test_multiplayer_ws_realtime.py`. Broader abandoned-room cleanup still needs more playtest coverage.
