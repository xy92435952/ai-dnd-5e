...
| [ ] | P1 | Stealth and perception procedures | Mostly skill-check/narrative. | Passive perception, group stealth, surprise, hidden condition, light, and noise are rule-backed. |
...
5. P1 monster traits: multiattack, resistances, immunities, condition immunity, recharge. First slices complete for condition immunity, multiattack turn limits, Pack Tactics, damage-type Recharge action execution, breath-style Recharge multi-target damage, and Legendary Resistance against failed enemy spell/control/reaction saves; complex Recharge rider effects, precise template targeting, and legendary/lair actions remain.
