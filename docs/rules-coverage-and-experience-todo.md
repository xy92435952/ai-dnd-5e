...
- [~] Add target validation before submit, not only backend error after click. 2026-05-30: skill bar now honors backend skill metadata (`requires_target`, `target_type`, and nested targeting hints) before submit for common target-required actions; spell cast modal now pre-blocks missing targets and missing AoE center points before submit, but full ground-point placement/confirmation still needs deeper validation.
