...
| [~] | P1 | Traps and investigation | Backend passive discovery exists for hidden traps, secret doors, clues, and mechanisms. Triggered trap resolution rolls saves and damage, attack-roll traps compare configured attack bonuses to target AC, `state_delta` can apply trigger/attack/disarm outcomes to persisted characters, and disarms record trap state. Richer persistent trap state remains. | Detect, disarm, trigger, save/attack, damage, and persistent trap state are represented. |
