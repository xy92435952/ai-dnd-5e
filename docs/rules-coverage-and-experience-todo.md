...
| [~] | P1 | Traps and investigation | Backend passive discovery exists for hidden traps, secret doors, clues, and mechanisms. Triggered trap resolution now rolls configured save and full/half damage without mutating HP, ready for state/applicator integration. Disarm, attack-roll traps, applied damage, and persistent trap state remain. | Detect, disarm, trigger, save/attack, damage, and persistent trap state are represented. |
