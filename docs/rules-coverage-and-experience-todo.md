...
| [ ] | P2 | Ammunition and thrown weapons | Inventory has ammo fields; combat trust unknown. | Ranged attacks consume ammo/thrown weapons and reject unavailable equipment. |
...
| [~] | P1 | Traps and investigation | Backend passive discovery exists for hidden traps, secret doors, clues, and mechanisms. Triggered trap resolution rolls configured saves and full/half damage, `state_delta.trap_triggers` can apply trap damage/failed conditions to persisted characters, and `state_delta.trap_disarms` can resolve disarm checks, record trap state, and trigger failed attempts. Attack-roll traps and richer persistent trap state remain. | Detect, disarm, trigger, save/attack, damage, and persistent trap state are represented. |
...
- [ ] Verify members leaving all rooms cleans WS state and dissolves rooms correctly when host leaves last.
...
- [ ] Add regression prompts for DM output schema: narrative, needs_check, state_delta, player_choices, companion_reactions.
...
- [ ] Verify reaction prompt is visible only to the eligible reactor.
