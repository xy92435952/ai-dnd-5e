...
- [~] Verify every visible combat button either performs a legal action or shows a useful disabled/error state. 2026-05-30: Combat HUD turn controls, quick consumables, death saves, spell casting actions/tabs, skill bar entries, and Battle Master maneuver choices now expose disabled reasons or block illegal clicks with component coverage. Remaining work is a full page-level audit for battlefield cell interactions, reaction prompt edge states, forced end/return controls, and any class-specific skill-bar entries not covered by fixture data.
