...
| [~] | P1 | Critical hits | Weapon base dice already crit; 2026-05-30: weapon-hit rider dice now also double on crit for two-step, direct, and offhand attack paths, covering sneak attack, Divine Fury, Hex/Hunter's Mark/Divine Favor-style sustained riders, and Absorb Elements without doubling flat modifiers. Divine Smite now receives hit crit context from backend turn state and frontend prompt flow, rolling doubled smite dice on crit while preserving normal slot dice and regenerated OpenAPI/TS types. Covered by `backend/tests/unit/test_combat_attack_damage.py`, `backend/tests/unit/test_combat_direct_attack_service.py`, `backend/tests/unit/test_combat_features.py`, `backend/tests/integration/test_combat_endpoints.py`, `frontend/src/hooks/__tests__/useCombatAttackFlow.test.js`, and `frontend/src/hooks/__tests__/useCombatSpecialActions.test.js`. Remaining work: spell-attack crit dice. | Crit rules double dice, not flat modifiers, across weapons, spell attacks, smite, sneak attack, and riders. |
