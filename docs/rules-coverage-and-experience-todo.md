...
| [~] | P1 | Traps and investigation | Backend passive discovery exists for hidden traps, secret doors, clues, and mechanisms. Triggered trap resolution rolls configured saves and full/half damage, and a pure helper can apply trap damage/failed conditions to character-like targets. Disarm, attack-roll traps, DB state integration, and persistent trap state remain. | Detect, disarm, trigger, save/attack, damage, and persistent trap state are represented. |
