...
describe('Home responsive shell', () => {
  ...
  it('renders the module hub with responsive page, header, tabs, and card actions', async () => {
    ...
    const featuredCard = container.querySelector('.home-module-card.is-featured')
    expect(featuredCard).toBeInTheDocument()
    expect(featuredCard).toHaveAttribute('data-ready', 'true')
    expect(featuredCard.querySelector('.home-module-featured-tag')).toHaveTextContent('★ 推荐')
    expect(featuredCard.querySelector('.home-module-icon')).toHaveTextContent('📜')
    expect(featuredCard.querySelector('.home-module-title')).toHaveTextContent('Road to Candlekeep With An Exceptionally Long Module Name')
    expect(featuredCard.querySelector('.home-module-setting')).toHaveTextContent('A coastal road')
    expect(featuredCard.querySelector('.home-module-meta')).toBeInTheDocument()
    expect(featuredCard.querySelectorAll('.home-module-tag')).toHaveLength(2)
    expect(container.querySelector('.home-card-actions')).toBeInTheDocument()
    expect(featuredCard.querySelector('.home-module-delete')).toHaveClass('btn-ghost')
    const processingCard = container.querySelector('.home-module-card[data-ready="false"]')
    expect(processingCard).toBeInTheDocument()
    expect(processingCard.querySelector('.home-status-badge')).toHaveAttribute('data-processing', 'true')
  ...
  it('switches to saves with responsive save cards and wrapping metadata', async () => {
    ...
    expect(screen.getByRole('button', { name: '↩' })).toHaveClass('home-save-action')
