...
describe('CharacterSheet inventory integration', () => {
  ...
  it('renders responsive sheet shell and major stat grids', async () => {
    ...
    const hpStat = within(stats).getByRole('listitem', { name: '生命值 4 / 12' })
    expect(hpStat).toHaveClass('character-sheet-stat-card-hp')
    expect(hpStat).toHaveAttribute('data-hp-tone', 'wounded')
    const hpMeter = within(stats).getByRole('meter', { name: '生命值比例' })
    expect(hpMeter).toHaveAttribute('aria-valuenow', '4')
    expect(hpMeter).toHaveAttribute('data-hp-tone', 'wounded')
    const hpMeterFill = hpMeter.querySelector('.character-sheet-hp-meter-fill')
    expect(hpMeterFill).toBeInTheDocument()
    expect(hpMeterFill).toHaveStyle({ '--character-sheet-hp-width': '33%' })
    ...
    expect(container.querySelector('.character-sheet-bottom-spacer')).toBeInTheDocument()
