...
describe('Adventure render smoke', () => {
  ...
  it('多人同步断开时禁用发言入口并阻止行动提交', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Save checkpoint' })).toBeDisabled()
    expect(screen.getByRole('button', { name: 'Open rest menu' })).toBeDisabled()
    expect(screen.getByRole('button', { name: 'Prepare spells' })).toBeDisabled()
