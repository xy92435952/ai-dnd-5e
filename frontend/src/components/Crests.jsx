/**
 * 12 个职业的 SVG 纹章 + DM/敌人通用纹章
 * 配色由 .portrait-{cls} 在 CSS 中提供（components.css）
 * 每个纹章用 currentColor 着色，自动响应所属职业的主题色
 *
 * 来源：design v0.10 prototype crests.jsx
 */
...
export const Crest = {
  fighter: (
    <svg className="crest" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth={sw} {...lj}>
  ...
  druid: (
    <svg className="crest" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="2.3" {...lj}>
  ...
  monk: (
    <svg className="crest" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth={sw} strokeLinecap="round">
  ...
  dm: (
    <svg className="crest" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="2" {...lj}>
...
export function classKey(input) {
  if (!input) return 'fighter'
  const s = String(input).toLowerCase()
  const map = {
    战士: 'fighter', fighter: 'fighter',
    法师: 'wizard', wizard: 'wizard',
    牧师: 'cleric', cleric: 'cleric',
    游荡者: 'rogue', 盗贼: 'rogue', rogue: 'rogue',
    圣武士: 'paladin', 圣骑士: 'paladin', paladin: 'paladin',
    游侠: 'ranger', ranger: 'ranger',
    蛮战士: 'barbarian', 野蛮人: 'barbarian', barbarian: 'barbarian',
    吟游诗人: 'bard', bard: 'bard',
    德鲁伊: 'druid', druid: 'druid',
    术士: 'sorcerer', sorcerer: 'sorcerer',
    契约师: 'warlock', 邪术师: 'warlock', warlock: 'warlock',
    武僧: 'monk', monk: 'monk',
    dm: 'dm', 地下城主: 'dm',
    enemy: 'enemy', 敌人: 'enemy',
  }
  return map[s] || 'fighter'
}
