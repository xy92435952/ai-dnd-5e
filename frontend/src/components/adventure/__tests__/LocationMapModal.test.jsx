...
describe('LocationMapModal', () => {
  it('renders the current map without exposing hidden future locations or encounter details', () => {
    ...
    const status = screen.getByLabelText('Selected location status for Training Yard')
    expect(status).toHaveAttribute('aria-live', 'polite')
    expect(within(status).getByText('Focus')).toBeInTheDocument()
    expect(within(status).getByText('Current')).toBeInTheDocument()
    expect(within(status).getByText('Route')).toBeInTheDocument()
    expect(within(status).getByText('Current location')).toBeInTheDocument()
    expect(within(status).getByText('1 exit')).toBeInTheDocument()
    expect(within(status).getByText('No encounter templates')).toBeInTheDocument()
    ...
    const routeList = screen.getByRole('list', { name: 'Routes from Training Yard' })
  ...
  it('selects a different discovered location without changing the current marker', () => {
    ...
    const status = screen.getByLabelText('Selected location status for Gatehouse')
  ...
  it('shows selected-location exit gates without revealing hidden routes', () => {
    ...
    const status = screen.getByLabelText('Selected location status for Armory')
    expect(within(status).getByText('Gated route')).toBeInTheDocument()
    const travel = screen.getByLabelText('Route from current')
  ...
  it('selects an available public encounter template', () => {
    ...
    const status = screen.getByLabelText('Selected location status for Training Yard')
    expect(within(status).getByText('1 encounter template')).toBeInTheDocument()
    const encounters = screen.getByLabelText('Selected encounter templates')
    expect(encounters).toHaveAttribute('aria-live', 'polite')
    expect(within(encounters).getByText('Can arm this encounter for the next combat.')).toBeInTheDocument()
    expect(within(encounters).getByRole('group', { name: 'Encounter action for Construct Patrol' })).toBeInTheDocument()
  ...
  it('shows public encounter environment pressure without private details', () => {
    ...
    expect(within(encounters).getByText('Already armed for this location.')).toBeInTheDocument()
    expect(screen.queryByText('sparking conduit')).not.toBeInTheDocument()
  ...
  it('previews public encounters on a selected non-current visible location', () => {
    ...
    const mapped = screen.getByRole('group', { name: 'Mapped encounters' })
    ...
    const status = screen.getByLabelText('Selected location status for Armory')
    expect(within(status).getByText('Selected')).toBeInTheDocument()
    expect(within(status).getByText('Reachable')).toBeInTheDocument()
    expect(within(status).getByText('1 encounter template')).toBeInTheDocument()
    const encounters = screen.getByLabelText('Selected encounter templates')
