  ...
  return (
    <div style={{ padding: 24, height: '100%', overflow: 'auto' }}>
      <div style={{ textAlign: 'center', marginBottom: 18 }}>
        <div className="eyebrow">◈ 英雄铸造 ◈</div>
        <div className="display-title" style={{ fontSize: 26, marginTop: 4 }}>选择你的英雄之路</div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, maxWidth: 1100, margin: '0 auto' }}>
        {/* 左 — 职业选择网格 */}
        <div>
            ...
            {ALL_CLASSES.map(c => (
              <div
                ...
                style={{
                  ...
                  borderColor: c.key === cls ? 'var(--amber)' : undefined,
                  boxShadow: c.key === cls ? '0 0 18px -4px rgba(212,168,71,.6)' : undefined,
                  transition: 'var(--transition)',
                }}
              >
                <Portrait cls={c.key} size="sm" style={{ margin: '0 auto 8px' }} />
                <div style={{ fontFamily: 'var(--font-heading)', fontSize: 12, color: 'var(--parchment)' }}>{c.name}</div>
              </div>
            ))}
          </div>
        </div>
        {/* 右 — 预览 */}
        <div className="panel-ornate" style={{ padding: 22, textAlign: 'center', position: 'sticky', top: 0 }}>
          <Portrait cls={cls} size="xl" style={{ margin: '0 auto 14px' }} />
          <div className="display-title" style={{ fontSize: 22 }}>{ALL_CLASSES.find(c => c.key === cls)?.name}</div>
          <Divider>✦</Divider>
          <div style={{ fontFamily: 'var(--font-script)', fontStyle: 'italic', color: 'var(--parchment-dark)', fontSize: 13, marginBottom: 14 }}>
            "{ALL_CLASSES.find(c => c.key === cls)?.desc}"
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, textAlign: 'left' }}>
            <input className="input-fantasy" placeholder="英雄之名" defaultValue="艾琳·晨光" />
            <select className="input-fantasy" defaultValue="半精灵">
              {['人类', '精灵', '矮人', '半身人', '半精灵', '龙裔', '提夫林', '侏儒'].map(r => <option key={r}>{r}</option>)}
            </select>
          </div>
          <button className="btn-gold" style={{ marginTop: 14, width: '100%' }}>✦ 铸造英雄 ✦</button>
        </div>
      </div>
    </div>
  );
