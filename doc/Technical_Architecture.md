...
│   ├── services/                 # 业务逻辑层
│   │   ├── __init__.py
│   │   ├── action_parser.py      # 自然语言→结构化行动解析器（AI 翻译玩家自由文本）
│   │   ├── ai_combat_agent.py    # AI 战斗决策 Agent（敌人策略 + 12+职业细分队友策略）
...
[analyze_player] ── 分析玩家角色（职业/子职/属性/装备）
  │
  ▼
[select_classes] ── 从 5 个角色池随机选择互补职业+子职业
  │                  （34 种职业/子职业组合，避免重复）
  │                  角色池：tank / healer / melee_dps / ranged_dps / support
  ▼
...
#### 队友策略 Prompt 设计（v0.9 大改：12+ 职业细分策略）
...
v0.9 职业细分策略（12+ 角色角色）：
- Cleric: 优先治疗（HP<50%队友 > 伤害输出），绝不近战
- Wizard/Sorcerer/Warlock: 远程法术输出，保持距离，绝不近战
- Paladin: 前排近战 + 神圣斩击 + 紧急治疗
- Fighter/Barbarian: 前排坦克，优先保护脆皮
- Ranger: 远程优先，利用地形优势
- Rogue: 寻找偷袭机会，灵巧动作脱身
- Bard: 支援/控制优先，灵感分配
- Druid: 根据子职业切换（Moon→野性形态，Land→法术控制）
- Monk: 高机动近战，Ki 技能灵活运用

关键规则：
- 施法职业（Wizard/Sorcerer/Warlock/Cleric/Druid/Bard）绝不使用近战攻击
- 牧师在有队友受伤时始终优先治疗
```
...
[Action Parser] ── action_parser.py
  ...
  ├─ 翻译为结构化行动列表：
  │     [{type:"move", x:5, y:3},
  │      {type:"attack", target_id:"spider_01", weapon:"torch"}]
  │
  ▼
[引擎逐步执行]
  ├─ 移动 → 更新位置 → 检测借机攻击
  ├─ 攻击 → 真实骰子判定（d20 + 修正 vs AC）
  ├─ 法术 → 法术位消耗 + 效果应用
  └─ 其他 → 按行动类型分发到对应引擎
  │
  ▼
